...
void MainScene::mousePressd(QMouseEvent *event)
{
    ...
    {
        // Создать новый тип строки и сохраните его в векторе элементов
    ...
    else if(draw_type == "Polygon")     // Многоугольник
    {
        ...
        else if(event->button() == Qt::RightButton) // Щелк правой кнопкой мыши, многоугольник будет нарисован
        {
            if(!polygonPoints.isEmpty())
            {
               // Удалить все временно нарисованные края
                ...
                // Добавить новый элемент типа многоугольника с существующими ключевыми точками
    ...
    else if(draw_type == "Text")        // Текст
    {
        ...
        if(!text.isEmpty())
        {
            ...
            textItem->setPen(*pen);
            brush_text->setColor(pen->color()); // Сохранить тот же цвет, что и линия, каждый раз, когда вы рисуете
