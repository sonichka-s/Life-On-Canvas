    ...
    , ui(new Ui::MainWindow)
{
    ...
    /* Установите вид в качестве центрального элемента управления окна.
    ...
    /* Инициализируйте размер окна, примечание: в дополнение к центральной области 
    все окно также содержит такие области, как панель меню, панель инструментов
     и строка состояния. */
    this->resize(800, 600);     

   // Инициализация кистей и кистей
    pen = new QPen();
    scene->setPen(pen);
   ...
   // Просто добавьте начальный текст
...
{
    Q_UNUSED(event); // Объявляйте неиспользуемые переменные, чтобы избежать предупреждений компилятора
   /* Если сцена близка к размеру представления, будет активирована 
    ...
    scene->setRect(QRect(0, 0, view->width(), view->height()));   // Обновите прямоугольную область чертежа, которую необходимо обновить
}
...
void MainWindow::init_action_groups()
{
   ...
   /* Все типы линий кисти объединяются в одну группу, а 
    ...
    /* Все типы заливки объединяются в одну группу, и ни один
     тип заливки не задается в качестве типа по умолчанию */
...
{
    // Обновите строку строки состояния, информация строки состояния, связанная с каждой строкой, автоматически обновит содержимое
...
{
   // Установите текущий режим рисования
